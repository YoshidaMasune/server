...
export const createNewOne = async (req: WorkRequest, res: Response, next: NextFunction) => {
...
export const deleteOne = async function (req: Request, res: Response, next: NextFunction) {

    try {
        const workDel = Work.deleteOne({ _id: req.params.id }).exec().then(() => {
...
export const updateOne = (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
        ...
        Work.updateOne({ _id: req.params.id }, { ...req.body.work }).exec().then(re => {
            res.json(re)
        }).catch(err => {
            console.log(err)
            res.send(err)
...
export const getById = async (req: Request, res: Response, next: NextFunction) => {
    try {
        const data = await Work.findById({_id: req.params.id}).populate('user').populate('address').exec();
